...
let searchResults = [];
let isFullArticle = false; // Track whether it's displaying full article or summary
...
async function fetchPageContent(title) {
    ...
    try {
        ...
        document.getElementById("content-section").innerHTML = `
            ...
            <button onclick="toggleFullArticle('${data.title}')">${translations[currentLanguage].readMore}</button>
...
// Toggle between full article and summary article
function toggleFullArticle(title) {
    if (isFullArticle) {
        fetchPageContent(title); // Fetch and display the summary again
    } else {
        fetchFullPageContent(title); // Fetch and display the full article
    }
    isFullArticle = !isFullArticle; // Toggle the state
}
...
async function fetchFullPageContent(title) {
    ...
    try {
        ...
        document.getElementById("content-section").innerHTML += `
            ...
            <button onclick="toggleFullArticle('${title}')">${translations[currentLanguage].readMore}</button>
        `;
    } catch (error) {
